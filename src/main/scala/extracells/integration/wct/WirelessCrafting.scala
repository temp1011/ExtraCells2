...
object WirelessCrafting {
  def openCraftingTerminal(player: EntityPlayer, slot: Int): Unit = WCTApi.instance.openWCTGui(player, false, slot)

  def getBoosterItem = new ItemStack(Item.getByNameOrId("wct:infinity_booster_card"))

  def isBoosterEnabled: Boolean = WTApi.instance().getConfig.isInfinityBoosterCardEnabled
